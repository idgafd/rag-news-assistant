...
async def load_model():
    ...
    try:
        if (os.path.exists(model_path)
            ...
            # model = CrossEncoder("microsoft/MiniLM-L12-H384-uncased", max_length=512, device='cpu')
...
async def test_model():
    """Test model with minimal data"""
    if model is None:
        raise HTTPException(status_code=500, detail="Model not loaded")

    try:
        logger.info("Testing model with simple data...")

        test_pairs = [['machine learning', 'AI is powerful'], ['machine learning', 'Weather is nice'], ['machine learning', 'Neural networks learn patterns']]

        logger.info("About to call model.predict...")
        scores = model.predict(test_pairs, batch_size=1, show_progress_bar=False)
        logger.info(f"model.predict completed, scores: {scores}")

        logger.info(f"Converting to float: {float(scores[0])}")

        return {"status": "success", "test_score": float(scores[0])}
